...
public class Implementor implements Impler, JarImpler {
     ...
     * there writes an implementation of a given class/interface to a {@code .java} file.
     * </p>
     *
    ...
    public void implement(Class<?> token, Path root) throws ImplerException {
        ...
        try (BufferedWriter writer = Files.newBufferedWriter(sourcePath)) {
            writer.write(normalize(getImplementation(token)));
        } catch (IOException e) {
    ...
    /**
     * Normalizes text traversing each symbol checking its {@link Integer} value.
     * <p>
     * It a symbol {@link Integer} value is less than {@code 128},
     * it stays the same and is added to a resulting string.
     * Otherwise, it is justified to a normal encoding and added
     * to a resulting string.
     * </p>
     *
     * @param text {@link String} text to normalize
     * @return normalized {@link String}
     */
    private static String normalize(String text) {
        final int BARRIER = 128;
        StringBuilder builder = new StringBuilder();
        for (char symbol : text.toCharArray()) {
            builder.append(symbol < BARRIER ?
